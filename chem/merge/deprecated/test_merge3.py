...
# Optional: pretty SMILES output using the project's known-good converter.
# If RDKit isn't installed or import paths differ, tests still run and print meta smiles.
try:
    from chem.build.molgraph_to_mol import molgraph_to_smiles  # type: ignore
...
def _smiles(g: MolGraph) -> str:
    """Best-effort SMILES for test output (RDKit if available; else meta)."""
    if molgraph_to_smiles is not None:
        s = molgraph_to_smiles(g)
        if s is not None:
            return str(s)
    return str(g.meta.get("smiles", ""))
...
def test_resolve_site_substituent() -> None:
    ...
    print(f"    input_smiles: {_smiles(g)}")
...
def test_merge_substituent_basic() -> None:
    ...
    print(f"    A_smiles: {_smiles(a)}")
    print(f"    B_smiles: {_smiles(b)}")
    print(f"    merged_smiles: {_smiles(merged)}")
