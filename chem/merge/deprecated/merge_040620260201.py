...
def _delete_atoms_with_map(g: MolGraph, atom_indices: Sequence[int]) -> Tuple[MolGraph, np.ndarray]:
    ...
    g2 = MolGraph(
        ...
        resonance_systems=[],  # per your updated policy: do not carry resonance systems
        cache=g.cache,
        dirty=g.dirty,
        meta=dict(g.meta),
        rdkit_mol=None,
...
def merge_by_labels(
    ...
    *,
    ...
    # Remove H for each anchor in A that will gain new bond(s)
    for anc in list(anchors_a):
        gA, m = _maybe_remove_one_explicit_h(gA, anc)
        # compose maps: new_map = m[old]
        mapA = m[mapA]
        # update anchors to post-delete indices
        anchors_a = [int(mapA[x]) if int(mapA[x]) >= 0 else -1 for x in anchors_a]
        ...
        # update dummy idx too
        site_a.dummy_idx = int(mapA[site_a.dummy_idx]) if int(mapA[site_a.dummy_idx]) >= 0 else -1
    ...
    mapA = m[mapA]
    anchors_a = [int(mapA[x]) for x in anchors_a]
    if any(x < 0 for x in anchors_a):
    ...
    for anc in list(anchors_b):
        gB, m = _maybe_remove_one_explicit_h(gB, anc)
        mapB = m[mapB]
        anchors_b = [int(mapB[x]) if int(mapB[x]) >= 0 else -1 for x in anchors_b]
        ...
        site_b.dummy_idx = int(mapB[site_b.dummy_idx]) if int(mapB[site_b.dummy_idx]) >= 0 else -1
    ...
    mapB = m[mapB]
    anchors_b = [int(mapB[x]) for x in anchors_b]
    if any(x < 0 for x in anchors_b):
